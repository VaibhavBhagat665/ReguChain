...
@app.get("/api/sanctions/check/{address}")
async def check_sanctions_address(address: str):
    """Check if an address is in sanctions database"""
    try:
        logger.info(f"Checking sanctions for address: {address}")
        
        # Search vector store for this specific address
        search_results = await vector_store.search(f"address {address} wallet sanctions OFAC", k=20)
        
        # Also search for partial matches
        partial_results = await vector_store.search(f"{address[:10]} {address[-10:]} sanctions", k=10)
        
        # Check if address appears in any documents
        matches = []
        for doc_tuple in search_results + partial_results:
            if isinstance(doc_tuple, tuple):
                doc, similarity = doc_tuple
            else:
                doc = doc_tuple
        ...
        # Get all documents in vector store for debugging
        all_docs = vector_store.get_all_documents()
        sanctions_docs = [doc for doc in all_docs if 'sanction' in doc.get('source', '').lower() or 'ofac' in doc.get('source', '').lower()]
        
        logger.info(f"Found {len(matches)} direct matches for {address}")
        logger.info(f"Total sanctions documents in database: {len(sanctions_docs)}")
        
        return {
            "address": address,
            "is_sanctioned": len(matches) > 0,
            "matches": matches,
            "total_sanctions_docs": len(sanctions_docs),
            "sample_sanctions_docs": [
                {
                    "source": doc.get('source', ''),
                    "content": doc.get('content', '')[:100] + "..."
                } for doc in sanctions_docs[:5]
            ]
        }
        
    except Exception as e:
        logger.error(f"Error checking sanctions for {address}: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/sanctions/add-test-data")
async def add_test_sanctions_data():
    """Add some test sanctioned addresses for demo"""
    try:
        # Known sanctioned addresses from public sources
        test_addresses = [
            {
                "address": "0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb4",
                "name": "Tornado Cash Router",
                "reason": "OFAC sanctioned mixer"
            },
            {
        ...
        documents = []
        for item in test_addresses:
            doc = {
                'id': f"test_sanction_{item['address']}",
                'source': 'OFAC_TEST_DATA',
                'text': f"OFAC Sanctioned Address: {item['address']} - {item['name']} - {item['reason']}",
                'timestamp': datetime.utcnow().isoformat(),
        ...
        await pathway_fallback_manager._process_documents(documents, 'TEST_SANCTIONS')
        
        logger.info(f"Added {len(documents)} test sanctioned addresses")
        
        return {
            "success": True,
            "message": f"Added {len(documents)} test sanctioned addresses",
            "addresses": [item['address'] for item in test_addresses]
        }
        
    except Exception as e:
        logger.error(f"Error adding test sanctions data: {e}")
        raise HTTPException(status_code=500, detail=str(e))
